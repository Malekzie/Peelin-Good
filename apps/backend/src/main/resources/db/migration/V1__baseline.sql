...
CREATE INDEX idx_customer_guest_expiry ON customer (guest_expiry_date) WHERE guest_expiry_date IS NOT NULL;
...
CREATE INDEX idx_batch_bakery ON batch (bakery_id);

-- Batch Inventory (junction)
...
CREATE INDEX idx_order_bakery ON "order" (bakery_id);
